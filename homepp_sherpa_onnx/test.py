...
from std_msgs.msg import String  # Assuming you want to publish String messages
...
class SpeechRecognitionNode(Node):
    def __init__(self):
        ...
        self.result_pub = self.create_publisher(String, 'recognition_result', 10)
    ...
    def init_recognizer(self):
        self.assert_file_exists(self.args['encoder'])
        self.assert_file_exists(self.args['decoder'])
        self.assert_file_exists(self.args['joiner'])
        self.assert_file_exists(self.args['tokens'])
    ...
    def main_loop(self):
        ...
        while rclpy.ok():
            ...
            if result and (last_result != result):
                ...
                # self.get_logger().info("{}:{}".format(segment_id, result))
    ...
    def message_sender(self):
        ...
        def on_open(ws):
            self.get_logger().info("WebSocket on open")
            while not get_result_event.is_set():
                time.sleep(1)
            self.get_logger().info("Send %s to %s", recording_results, server)
            ws.send(recording_results)
            get_result_event.clear()

        def on_message(ws, message):
            self.get_logger().info("Received from server: %s", message)
            on_open(ws)
...
def main(args=None):
    ...
    sender = threading.Thread(target=node.message_sender, name='messageSender')
    sender.start()
    node.main_loop()
    rclpy.shutdown()
