...
class KeywordsSpotterNode(Node):
    ...
    def __init__(self):
        ...
        # Setup sound device callback
        def callback(indata, frames, time, status):
            if status:
                self.get_logger().warning(f"Error in callback: {status}")
                return
            samples = indata.squeeze().astype('float32') / 32768.0
        ...
        # Start sound device stream
        with sd.InputStream(channels=1, dtype='int16', callback=callback, samplerate=self.sample_rate):
